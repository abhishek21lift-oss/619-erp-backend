 ...
 *
 * Cover banner and portfolio items are real columns but have no upload path
 * yet. Scoring them would park everyone below 100% with a step they cannot
 * take, which is worse than not counting them — a checklist you cannot finish
 * stops being read. They join this table in the same change that ships their
 * upload.
 *
 ...
 */
const WEIGHTS = [
  { key: 'avatar', weight: 8, tab: 'overview', label: 'Add a profile photo', done: (p) => Boolean(p.avatar_url) },
  ...
  { key: 'bio', weight: 10, tab: 'credentials', label: `Write a bio (${MIN.bio}+ characters)`, done: (p) => str(p.bio).length >= MIN.bio },
  ...
  { key: 'job_title', weight: 6, tab: 'overview', label: 'Add your job title', done: (p) => Boolean(str(p.job_title)) },
  { key: 'designation', weight: 5, tab: 'credentials', label: 'Add your designation', done: (p) => Boolean(str(p.designation)) },
  { key: 'experience_since', weight: 6, tab: 'credentials', label: 'Set when you started coaching', done: (p) => Boolean(p.experience_since) },
  { key: 'coaching_modes', weight: 4, tab: 'credentials', label: 'Say how you coach', done: (p) => list(p.coaching_modes).length >= 1 },
  { key: 'previous_gyms', weight: 5, tab: 'credentials', label: 'Add where you have coached', done: (p) => list(p.previous_gyms).length >= 1 },
  {
    key: 'working_hours',
    weight: 6,
    tab: 'credentials',
  ...
  { key: 'specialisations', weight: 8, tab: 'credentials', label: `Add ${MIN.specialisations} specialisations`, done: (p) => list(p.specialisations).length >= MIN.specialisations },
  {
    key: 'certifications',
    weight: 12,
    tab: 'credentials',
  ...
  { key: 'education', weight: 6, tab: 'credentials', label: 'Add your education', done: (p) => list(p.education).length >= 1 },
