...
router.get('/meals', auth, async (req, res, next) => {
  try {
    ...
    const { rows } = await pool.query(
      `SELECT * FROM meals WHERE ${conds.join(' AND ')} ORDER BY meal_type, name`,
      params
    );
    res.json(rows);
  } catch (err) {
...
router.post('/meals', auth, async (req, res, next) => {
  try {
    ...
    const { rows } = await pool.query(`
      ...
      [uuid(), d.name.trim(), d.description || null, d.meal_type || 'breakfast',
    ...
    if (Array.isArray(d.meals)) {
      for (const m of d.meals) {
        await pool.query(`
          ...
          VALUES ($1,$2,$3,$4,$5)`,
          [uuid(), id, m.meal_id, parseInt(m.day_of_week) || null, parseInt(m.sort_order) || 0]
        );
      }
    }
...
router.post('/assign', auth, async (req, res, next) => {
  try {
    ...
    const { rows } = await pool.query(`
      ...
      RETURNING *`,
      [uuid(), d.diet_template_id, d.client_id, req.user.trainer_id || null,
  ...
  try {
    ...
    const { rows } = await pool.query(`
      ...
      ON CONFLICT (client_id) DO UPDATE SET
        ...
        updated_at = NOW()
      RETURNING *`,
      [uuid(), clientId, d.goal || null, d.goal_other || null,
