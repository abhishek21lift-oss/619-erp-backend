...
router.get('/exercises', auth, async (req, res, next) => {
  try {
    ...
    const { rows } = await pool.query(
      `SELECT * FROM exercises WHERE ${conds.join(' AND ')} ORDER BY muscle_group, name`,
      params
    );
    res.json(rows);
  } catch (err) {
...
router.post('/exercises', auth, async (req, res, next) => {
  try {
    const d = req.body;
    if (!d.name?.trim())
    ...
    const { rows } = await pool.query(`
      ...
      [uuid(), d.name.trim(), d.description || null, d.muscle_group || 'Full Body',
...
router.put('/exercises/:id', auth, async (req, res, next) => {
  try {
    const d = req.body;
    const { rows } = await pool.query(`
...
router.delete('/exercises/:id', auth, async (req, res, next) => {
  try {
    const { rows } = await pool.query(
    ...
    if (Array.isArray(d.exercises)) {
      for (const ex of d.exercises) {
        await pool.query(`
          ...
          [uuid(), id, ex.exercise_id, parseInt(ex.day_of_week) || 1,
...
router.post('/assign', auth, async (req, res, next) => {
  try {
    ...
    const { rows } = await pool.query(`
      ...
      RETURNING *`,
      [uuid(), d.workout_plan_id, d.client_id, req.user.trainer_id || null,
