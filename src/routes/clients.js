...
router.get('/', auth, async (req, res) => {
  try {
    ...
    await pool.query(`UPDATE clients SET status='expired', updated_at=NOW()
...
router.post('/', auth, async (req, res) => {
  try {
    ...
    const { rows: last } = await pool.query(`SELECT client_id FROM clients ORDER BY client_id DESC LIMIT 1`);
    let clientId = 'FS0001';
    if (last[0]?.client_id) {
      const num = parseInt((last[0].client_id || 'FS0000').replace('FS', '')) + 1;
      clientId = 'FS' + String(num).padStart(4, '0');
    }

    const id = uuid();
    const base    = parseFloat(d.base_amount)   || 0;
    const disc    = parseFloat(d.discount)      || 0;
    const final   = parseFloat(d.final_amount)  || (base - disc);
    ...
    if (trainer_id && !trainer_name) {
      const { rows: tr } = await pool.query('SELECT name FROM trainers WHERE id=$1', [trainer_id]);
    ...
    await pool.query(`
      ...
      [id, clientId, d.name.trim(), d.mobile||null, d.email?.toLowerCase()||null,
       ...
       parseFloat(d.weight)||null, d.notes||null, d.status||'active']
    );

    // If paid > 0, auto-create a payment record
    if (paid > 0) {
      const { rows: tr } = await pool.query('SELECT name, incentive_rate FROM trainers WHERE id=$1', [trainer_id]);
      ...
      await pool.query(`
        ...
        [uuid(), id, d.name.trim(), trainer_id, trainer_name, paid,
         ...
         receiptNo, d.package_type||null, Math.round(paid * rate)]
      );
    }
...
router.put('/:id', auth, async (req, res) => {
  try {
    ...
    const disc    = parseFloat(d.discount)      ?? existing[0].discount;
    const final   = parseFloat(d.final_amount)  ?? existing[0].final_amount;
    const paid    = parseFloat(d.paid_amount)   ?? existing[0].paid_amount;
