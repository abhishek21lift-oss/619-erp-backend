    ...
    const query = async () => {
      const { rows: [row] } = await pool.query(`
        ...
        SELECT ok.total_studios, ok.active_studios, ok.pending_studios, ok.trial_studios, ok.suspended_studios,
          ...
          sk.active_subscriptions, sk.trial_subscriptions, sk.expiring_in_7d,
          rk.collected_30d_inr, pk.failed_payments_30d, ak.critical_alerts, ak.high_alerts, ak.medium_alerts
      ...
      return {
        ...
        platform_revenue: {
          // This field is retained for API compatibility. It is explicitly collected
          // revenue over 30d, not MRR; the UI must not label it MRR.
          mrr_inr: Number(r.collected_30d_inr ?? 0),
          collected_30d_inr: Number(r.collected_30d_inr ?? 0),
        ...
        security: {
          critical_alerts: r.critical_alerts ?? 0, high_alerts: r.high_alerts ?? 0,
