...
async function createMemberCode() {
  ...
  try {
    await client.query('SELECT pg_advisory_lock($1)', [2026052201]);
    const { rows } = await client.query(`SELECT COUNT(*) AS c FROM members`);
    const next = (parseInt(rows[0].c) + 1).toString().padStart(4, '0');
    return `FS${next}`;
  } finally {
    await client.query('SELECT pg_advisory_unlock($1)', [2026052201]);
    client.release();
  }
}
...
async function create(input, ctx) {
  // Auto-generate member_code: FS0001 style (atomic via advisory lock)
