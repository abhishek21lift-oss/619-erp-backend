 ...
 * - is scoped to exactly one organization: orgId is bound into every query
 *   with strict equality, so a missing org matches nothing rather than every
 *   studio (only tenant routes call this module; the platform console has its
 *   own queries),
...
async function getMonthlyRevenue({ year = new Date().getFullYear(), orgId = null } = {}) {
...
async function getDuesSummary({ high = 10000, medium = 3000, orgId = null } = {}) {
  const params = [];
  params.push(orgId);
  const orgIdx = params.length;
...
async function getDuesRows({ orgId = null, limit = 100 } = {}) {
  const params = [];
  params.push(orgId);
  const orgIdx = params.length;
...
async function getAttendanceStats({ from, to, granularity = 'day', orgId = null } = {}) {
  const { from: f, to: t } = defaultRange(from, to);
...
async function getAttendanceToday({ orgId = null } = {}) {
  const params = [];
  params.push(orgId);
  const orgIdx = params.length;
  const { rows } = await pool.query(
    `SELECT COUNT(*) FILTER (WHERE a.status='present')::int AS present,
            ...
            COUNT(*)::int AS total
       FROM attendance_logs a
      WHERE a.date = CURRENT_DATE AND a.ref_type = 'client'
        AND a.organization_id = $${orgIdx}`,
    params
  );
  const r = rows[0] || {};
 ...
 */
async function getRenewals({ from, to, orgId = null } = {}) {
  const { from: f, to: t } = defaultRange(from, to);
  const params = [f, t, orgId];
  const { rows } = await pool.query(
    `WITH cohort AS (
...
async function getRenewalRows({ orgId = null, days = 30, limit = 100 } = {}) {
...
/**
 * Returns every headline metric for the same {from,to,org} window in a
 ...
 */
async function getOverview({ from, to, year, orgId = null } = {}) {
  const { from: f, to: t } = defaultRange(from, to);
  const [revenue, dues, attendance, renewals, utilisation, monthly, trainers] = await Promise.all([
    getRevenue({ from: f, to: t, orgId }),
    getDuesSummary({ orgId }),
    getAttendanceStats({ from: f, to: t, orgId }),
    getRenewals({ from: f, to: t, orgId }),
    getUtilization({ orgId }),
    getMonthlyRevenue({ year: year || new Date(t).getFullYear(), orgId }),
 ...
 */
async function getSelfAttendanceHistory({ refId, refType, orgId = null, limit = 90 } = {}) {
  const cap = Math.min(Math.max(Number(limit) || 90, 1), 365);

  // Every query here carries the organization as well as the person: the
  // person's id alone is not the tenant boundary.
  const { rows: history } = await pool.query(
    `SELECT date, status, check_in_time, check_out_time, method, duration_minutes
       FROM attendance_logs
      WHERE ref_id = $1 AND ref_type = $2 AND organization_id = $4
      ORDER BY date DESC
      LIMIT $3`,
    [refId, refType, cap, orgId]
  );
  ...
  const { rows: agg } = await pool.query(
    `SELECT COUNT(*)::int AS total_days,
       ...
       FROM attendance_logs a
      WHERE a.ref_id = $1 AND a.ref_type = $2 AND a.organization_id = $4`,
    [refId, refType, CHECKED_IN_STATUSES, orgId]
  );
