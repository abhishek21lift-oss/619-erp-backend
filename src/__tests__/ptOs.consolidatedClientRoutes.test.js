...
let mockUser = { id: 'u1', role: 'trainer', is_owner: true, organization_id: ORG_A };
jest.mock('../middleware/auth', () => ({
  auth: (req, _res, next) => { req.user = mockUser; next(); },
  adminOnly: (req, res, next) => (req.user.role === 'trainer' && req.user.is_owner === true ? next() : res.status(403).json({ error: 'Forbidden' })),
...
beforeEach(() => {
  ...
  mockUser = { id: 'u1', role: 'trainer', is_owner: true, organization_id: ORG_A };
});
